...
def fix_csp(dist_dir):
    ...
    if os.path.exists(init_js):
        ...
        # NOTE: hand-written boot code (maximize/restore/minimize handlers +
        # theme-follow) now lives in wasm/chat-sidebar/sidebar-boot.js, loaded
        # via its own <script> tag in the trunk source index.html. Do NOT
        # inject handlers here anymore - init.js stays a pure wasm loader.
