...
1. **Decide how completion is judged.** If the goal is machine-verifiable, add a programmatic `check` (see above) — then no evaluator model is needed and you can skip the model question entirely. Otherwise (subjective goal) ask the user which **model** should evaluate. A model evaluator MUST be a direct-API provider (e.g. `anthropic`, `openai`, `gemini`); ACP providers (`claude-code`, `gemini-cli`, `kimi`, `openclaw`) cannot act as a model judge. **If the only providers available are ACP / there is no API key, use a `check`** — that is the supported path for that situation.
